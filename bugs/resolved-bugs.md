...
### 48. Switching the "Spec:" dropdown back to "Auto-detect" after picking a manual spec silently did nothing (T20b)
- Status: Solved
- Discovered: 2026-07-16 (queue.md's `T20b`: "'Auto-detect' stops working once a spec has been manually selected... switching back to 'Auto-detect' afterward doesn't re-engage talent-point detection")
- Version introduced: 0.38 (the manual spec-override dropdown itself, `bugs/resolved-bugs.md` #37) -- present since that dropdown's first build, only surfaced now by direct testing.
- Summary: `InitializeSpecDropdown` (`UI.lua`) set `info.value = nil` for the "Auto-detect" menu entry, relying on `OnSelect` reading it back as `self.value` (nil) to clear the override. This client's own `UIDropDownMenu_AddButton` doesn't leave `button.value` as nil in that case -- it falls back to `button.value = info.text` whenever `info.value` is nil -- so `self.value` on that button actually read back as the string `"Auto-detect"`. That string got passed straight to `Settings.SetSpecOverride`, which validates its argument against the class's real spec keys, found no match, and silently returned without changing anything -- so the override never cleared and `DetectSpec` kept using the stale manual pick.
- Investigation: Confirmed the exact fallback behavior (not a guess) by reading the real `UIDropDownMenu_AddButton` implementation bundled with two other installed, actively-used addons on this client -- ShamanPower's and PallyPower's own `LibUIDropDownMenu.lua` -- both containing the identical `if (info.value ~= nil) then button.value = info.value elseif (info.text) then button.value = info.text else button.value = nil end` logic.
- Resolution: Stopped reading `self.value` back in `OnSelect` entirely. Each dropdown button's `info.func` is now a closure that captures the intended spec key directly (`nil` for Auto-detect, `option.key` for a real spec), sidestepping the `button.value` quirk altogether.
- Validation: `luac -p` clean on `UI.lua`. Confirmed working live ("Autodetect is working").
- Follow-up: None.

### 49. Direct-entry stat weights accepted any number, including negative, with no bound and no warning (T23/T24)
- Status: Solved
- Discovered: 2026-07-15 (v0.384 test pass, T24: "It does allow me to input a negative number")
- Version introduced: 0.306 (the box's original 0-10 clamp was deliberately removed there, `bugs/resolved-bugs.md` #33, but nothing replaced it with a real bound or any rejection feedback).
- Summary: `Weights.SetWeightValue` stored whatever `tonumber()` returned from the edit box with no range check at all -- a typed negative number, or an absurdly large one, was accepted and used exactly as typed, with no explanation of what a valid value even looks like.
- Investigation: Not a regression to root-cause -- a genuine missing feature identified directly by the tester's own report and `queue.md`'s T23/T24 item (expected: "a positive number 0-20, rounding to the nearest tenth, and a dialog explaining that when a bad value is rejected -- not a silent revert").
...
### 55. SuggestionsUI window showed an empty box -- no rows, buttons, or text visible
- Status: Solved
...
- Summary: The window's backdrop/border was visible and its height varied correctly with candidate count, but the title, buttons, and every row's text were all invisible, even when `Suggestions.GetCandidates` was confirmed (via the debug log, same session) to have returned real, valid candidate data and the window logged `shown=true` with correct width/height/position/strata every time.
- Investigation: Ruled out a genuine engine bug (bugs #50-#52, confirmed fixed with live evidence), the window never actually being shown (log confirmed `IsShown()=true`, correct size and position after bug #53's drag-removal fix), and a Lua error aborting the populate function (none ever appeared in the log, and its own final "window shown" line -- written after the row loop and `window:Show()` -- fired every time). A full manual re-read of the anchor chain didn't turn up a certain cause either, so rather than guess further, added direct geometry/visibility logging for the first row (`GetLeft`/`GetTop`/`GetWidth`/`GetHeight`/`IsVisible`/`GetFrameLevel`/`GetFrameStrata`/`GetAlpha`/its own `nameText:GetText()`) and fixed one real issue found during that re-read: `rowParent` had never had an explicit height set (only `SetWidth`).
- Resolution: Confirmed fixed by the next live test. The new diagnostic log shows fully normal geometry -- `row1 isShown=true isVisible=true left=488.6... top=519.5... width=388.0 height=46.0 alpha=1 nameText="Finely Wrought Scale Leggings"` -- and real item names/candidates now render correctly across multiple slots. Honest note: giving `rowParent` an explicit height (previously only `SetWidth`) is the most likely actual fix among what changed between the last "still broken" report and this one, but bug #53's drag-removal landing fully (a clean, un-dragged, centered window) may also have been a necessary precondition -- both changes shipped together, so which one was strictly necessary isn't fully isolated. Not asserting more certainty than the evidence supports.
- Validation: `luac -p` clean on `SuggestionsUI.lua`. Confirmed working live ("It is fixed").
- Follow-up: None.

### 56. No way for a player to send a caught error or debug dump to the developer
- Status: Solved
- Discovered: 2026-07-17 (tester feedback T3). Filed as a tester-feedback gap rather than a defect -- nothing was broken, the capability simply did not exist.
- Version introduced: N/A -- missing capability, not a regression.
- Summary: A player who hit a Lua error or wanted to report a problem had to copy debug output out of chat by hand, line by line. There was no assisted path from "something went wrong" to "the developer has the information."
- Investigation: Established the hard constraint first: the WoW addon sandbox has **no network, file, or OS access** (already documented in `Debug.lua`'s own header), so an addon cannot send anything anywhere. "Send a report to the developer" is therefore impossible by construction; the real, achievable goal is removing the copy/paste friction.
- Resolution: Added `Debug.BuildReportText` (version + character context + the full debug log, assembled into one block) and `UI.ShowReportWindow` (a scrollable, effectively read-only multiline box with the text pre-selected and focused, the developer's email shown, and an explicit note that nothing is sent automatically). Reachable three ways: `/lgs report`, a "Copy report for developer" button in the settings window, and a once-per-session auto-offer popup when `SafeCall` catches a Lua error. The Lua-error chat hint now points at `/lgs report`. The once-per-session throttle exists because an error can fire repeatedly (e.g. from an `OnUpdate` path) and an unthrottled popup would spam the screen.
- Validation: `luac -p` + `luacheck` clean on `Debug.lua`/`UI.lua`/`Core.lua`. Confirmed working live by the author. Later independently validated in real use: the on-disk debug log from a subsequent session shows `report: auto-offering error report (first caught error this session)` firing on a genuine `script ran too long` error, and correctly suppressing the second occurrence.
- Follow-up: `/lgs testerror` and the `report` debug channel added alongside this are development/tester scaffolding, recorded as no-ship in `PACKAGING.md`.

### 57. Debug-enable confirmation did not say how to turn debug back off
- Status: Solved
- Discovered: 2026-07-17 (tester feedback T3 notes).
- Version introduced: N/A -- wording gap present since the debug toggle was added.
- Summary: `/lgs debug` confirmed that logging had been enabled and pointed at `/lgs debug dump`, but never told the player that running `/lgs debug` again turns it back off, leaving no discoverable way out of debug mode.
- Resolution: The enable confirmation now reads "Debug logging enabled (level N). Use /lgs debug dump to view recent entries, or /lgs debug again to turn it back off." One-line message change in `Debug.SetDebugEnabled`.
- Validation: `luac -p` + `luacheck` clean on `Debug.lua`. Confirmed working live by the author.
- Follow-up: None.

### 58. Suggestion scans re-ran on every instance loading screen, causing "script ran too long" Lua errors
- Status: Solved
- Discovered: 2026-07-21, from the on-disk debug log -- three real `script ran too long` errors (`Scoring.lua:266`, `Scoring.lua:194`, and one in Blizzard's own `Deprecated_Specialization_TBC.lua:51`), alongside per-slot scans firing about one second apart while the player was inside a dungeon (`continent=nil instanceId=389`).
- Version introduced: v0.44 (`Suggestions.lua`'s trigger block, shipped with the recommendation engine).
- Summary: WoW's watchdog was aborting the scoring code mid-execution. The obvious reading was a performance problem in `Scoring.lua`, but the real fault was upstream: the scan was running constantly in conditions where it was never meant to run at all.
- Investigation: `Suggestions.GetPlayerLocation` returns `MAP_ID_CONTINENT[instanceId]`, and no instance map id has an entry in that table -- so it returns `nil` for **any** instance. The trigger block then used `lastKnownContinent == nil` to mean "first check of the session", overloading `nil` as both "not yet checked" and "currently in an instance". Consequences: entering a dungeon read as a continent change (`0 -> nil`) and triggered a full 17-slot rescan; leaving read as another change (`nil -> 0`) and triggered another; and while inside, `lastKnownContinent` stayed `nil`, so every subsequent `PLAYER_ENTERING_WORLD` -- which fires on *every* loading screen, including releases and summons -- looked like a fresh "first check" and rescanned again. A dungeon run is therefore the worst case, which is exactly where the errors were logged. Root-caused per direct instruction to establish *why* it was triggering before optimising anything ("it was running in conditions where we didn't need it to run").
- Resolution: Split the two meanings apart. Added a separate `hasRunInitialScan` flag for "first check of the session", so `nil` no longer carries that meaning. Instance loading screens (`continent == nil`) now return early without touching `lastKnownContinent` at all -- a player's gear needs do not change because they zoned into a dungeon. On the first real (non-instance) `PLAYER_ENTERING_WORLD` of a session, added a record check (`HasSuggestionRecord`): scan only if `characterState.suggestionMemory` is missing or empty. Per direct instruction the record is deliberately **not** re-checked mid-session -- a record lost while logged in rebuilds on the next relog, not immediately. Equip / spec-change / level-up triggers were left untouched; those were always intended.
- Validation: `luac -p` + `luacheck` clean on `Suggestions.lua`.
- Follow-up: The per-slot scan is still one contiguous call over hundreds of items, which remains capable of tripping the watchdog on its own. Splitting that work into short-lived chained executions (each `C_Timer.After` callback gets a fresh runtime budget, confirmed available on this client via Attune/Auctionator/Questie/DBM-Core) is tracked separately in `queue.md` and is not part of this fix.
