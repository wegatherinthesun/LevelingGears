...
- Attempts to fix:
  ...
  - (v0.311) Expanded the same log lines to also capture `frame:GetScale()`, `frame:GetEffectiveScale()`, and `UIParent:GetEffectiveScale()` at both save and apply time. Reasoning: a "consistently offset, not random" symptom is exactly the signature a deterministic UI-scale mismatch between the drag session and a later login would produce (a real, known category of WoW addon position bugs) — this is pure additional diagnostic capture, no behavior change, so if that theory is right the next debug dump proves or disproves it immediately instead of needing a third round-trip to ask for more data.
- Resolution: Not confirmed — this is a diagnostic pass, not a verified fix.
- Validation: `luac -p UI.lua` and `luacheck UI.lua` both pass.
- Follow-up: Next test pass: drag the window to a deliberately memorable spot (e.g. flush against a corner of another native UI element), note the exact spot, `/reload`, and include the `/lgs debug dump` output covering both the drag (SaveWindowPosition log line) and the reopen (ApplySavedPosition log line) in the report — including the new scale values, in case they turn out to differ between the two.
- Update (2026-07-14, read directly from the on-disk `WTF/Account/ANDROIDEYES/SavedVariables/LevelingGears.lua` debug log): 37 position log lines captured. 36 of them are the window sitting at its untouched default (`point=CENTER relativePoint=CENTER x=0 y=0`) across many open/close cycles, with `frameScale`/`frameEffScale`/`uiParentEffScale` all exactly `1.0000` in every single one — **this rules out the UI-scale-mismatch theory the v0.311 logging was added to test**: there is zero scale variance anywhere in this data. The 37th (final) line is a real drag: `SaveWindowPosition: point=TOPLEFT relativePoint=TOPLEFT x=0 y=0` (also scale 1.0 throughout), which matches the currently-persisted `settings.position` table in the same file exactly — confirming the *save* path itself is not corrupting or mis-recording anything. However, there is no corresponding `ApplySavedPosition` line logged *after* this drag (the dump was captured immediately after dragging, before any reload/reopen) — so the one genuinely useful new data point (does this TOPLEFT/TOPLEFT/0,0 position restore to the exact same visual spot on the next open?) is still missing. This is a real, deliberate-looking test drag (an exact screen corner, matching this bug's own suggested test method) — the next debug dump just needs to be taken *after* reopening the window at least once following this drag, not immediately after the drag itself.
